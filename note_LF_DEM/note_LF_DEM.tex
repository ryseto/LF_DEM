...
\begin{align*}
\bm{F}_{\mathrm{H}} 
&= - \tens{R}(\bm{U}-\bm{U}^{\infty})  +  \tens{G} \tens{E}^{\infty}  \\
...
%
We may introduce a parameter $s$: $0<s<1$
for the diagonal matrix $\tens{R}_{\mathrm{onebody}}$.
%
\begin{equation}
...
\begin{equation}
 s \approx \frac{(\phi_{\mathrm{max}} - \phi)}{\phi_{\mathrm{max}}}
\end{equation}
...
We can find some implementation
Kumar's thesis Chapter 6 
(Fast lubrication dynamics algorithm)~\cite{Kumar_2010a}.
He extract the optimal values by comparing with Stokesian dynamics.
\begin{equation}
 \tens{R}_{\mathrm{0}}
=
\begin{pmatrix}
...
\begin{figure}
\centering
\includegraphics[width=10cm]{kumars_Rfu0.pdf} 
\caption{
