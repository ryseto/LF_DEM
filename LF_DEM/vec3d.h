...
public:
	...
	inline friend vec3d
	operator + (const vec3d &a1, const vec3d &a2){
		return vec3d(a1.x+a2.x, a1.y+a2.y, a1.z+a2.z);
	}
	
	inline friend vec3d
	operator + (const vec3d &v){
		return v;
	}
	...
	/* multiplication */
	inline friend vec3d
	operator * (const double &d, const vec3d &v){
    ...
    /* vector product */
	inline friend vec3d
	cross_vec_array(const vec3d &v1,
					const double *v2p){
	...
	inline friend double
	dist(const vec3d &a1, const vec3d &a2){
		return (a1-a2).norm();
	}
	...
	inline void
	rotateInfinitesimal(const vec3d &dphi){
		/* dphi must be small vector. */
		(*this) += cross(dphi, *this);
	}
	
	inline vec3d
	product_rate_of_strain(double * E){
