...
//    vec3d vj;
    ...
    for (int i=0; i < np; i++){
		int i3 = 3*i;
		vec3d dr(v_Brownian_init[i3]*dt_mid,
    ...
    for (int i = 0; i < np; i++){
		...
		relative_velocity_brownian[i].y = (v_Brownian_init[i3+1]
										   + 0.5*dt_ratio*( v_Brownian_mid[i3+1] - v_Brownian_init[i3+1]))*zero_2Dsimu;
